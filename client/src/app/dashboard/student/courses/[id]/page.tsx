...
export default async function CourseDetailPage({ params }: CourseDetailPageProps) {
  ...
  const progressRes = await getLessonProgresses(id, token);
  ...
  const allCompleted = lessons.length > 0 && lessons.every((l) => completedLessonDocIds.has(l.documentId));
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-5">
          ...
          <Card className="border border-border/40 bg-card/50 backdrop-blur-xl relative overflow-hidden">
            <div className="absolute top-2 right-2 text-[9px] font-mono opacity-15 text-primary">progress.json</div>
            ...
            <CardContent className="space-y-4">
              <div className="text-center pb-2 border-b border-border/30">
                <span className="text-4xl font-bold font-mono text-foreground">{completedCount}</span>
                <span className="text-muted-foreground text-sm ml-1">/ {lessons.length} লেসন</span>
              </div>
              <ProgressBar
          ...
          {/* Quiz unlock card */}
          <Card className="border border-border/40 bg-card/50 backdrop-blur-xl relative overflow-hidden">
            <div className="absolute top-2 right-2 text-[9px] font-mono opacity-15 text-primary">quiz.exe</div>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-bold flex items-center gap-1.5">
              ...
              {quiz ? (
                <>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    সব লেসন সম্পন্ন করলে কুইজ আনলক হবে। পাসিং স্কোর: <span className="font-semibold text-foreground font-mono">{quiz.passingScore}%</span>
                  </p>
                  <div className="text-[10px] font-mono text-muted-foreground space-y-1">
                    <div>সময়সীমা: {Math.floor(quiz.timeLimit / 60)} মিনিট</div>
