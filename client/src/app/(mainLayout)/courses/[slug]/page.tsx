...
import { getPublicCourseBySlug, checkEnrollmentStatusAction, enrollCourseAction } from "../../_components/_actions/coursesActions";
...
export default function PublicCoursePage({ params }: CourseDetailPageProps) {
  ...
  if (isLoadingCourse) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center py-32 bg-grid-cyber">
        ...
        <span className="font-mono text-xs text-muted-foreground mt-4">loading_course_data.log...</span>
      </div>
    );
  }
  ...
  if (!course) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center py-32 text-center bg-grid-cyber">
        ...
        <h2 className="text-xl font-bold text-foreground">কোর্সটি খুঁজে পাওয়া যায়নি!</h2>
        <p className="text-sm font-mono text-muted-foreground mt-2">Error 404: course_not_found</p>
        <Link href="/courses" className="mt-6">
          <Button variant="outline" size="sm">কোর্স ক্যাটালগে ফিরুন</Button>
        </Link>
      </div>
    );
  }
  ...
  const handleBKashPayment = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error("পেমেন্ট সম্পূর্ণ কিন্তু এনরোলমেন্ট ব্যর্থ হয়েছে। অনুগ্রহ করে হেল্পডেস্কে যোগাযোগ করুন।");
      }
    } catch (err) {
      toast.error("সার্ভার ত্রুটি ঘটেছে।");
    } finally {
    ...
    <main className="flex flex-1 flex-col pb-16 bg-grid-cyber relative">
      ...
      <div className="mx-auto max-w-5xl xl:max-w-6xl w-full px-4 pt-8 space-y-8">
        ...
        <div className="space-y-3">
          <h1 className="text-3xl font-extrabold tracking-tight font-heading text-foreground sm:text-4xl">{course.title}</h1>
          <p className="text-base text-muted-foreground max-w-3xl leading-relaxed">{course.description}</p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="border border-border/30 bg-card/10 backdrop-blur-md">
              ...
              <CardContent className="space-y-6 text-sm text-muted-foreground leading-relaxed">
                <p>
                  এই প্রোগ্রামটি বিশেষভাবে সাজানো হয়েছে প্রোগ্রামিংয়ের কোর কনসেপ্ট সহজে ও প্র্যাক্টিকালি বোঝার সুবিধার্থে। প্রতি সপ্তাহে থাকবে চ্যালেঞ্জিং টাস্ক ও লাইভ সেশন, যার মাধ্যমে আপনি সরাসরি কনসেপ্টগুলো প্রয়োগ করতে পারবেন।
                </p>

                {course.tags && course.tags.length > 0 && (
                  <div className="space-y-2">
                    <h4 className="text-xs font-mono text-foreground uppercase tracking-wider">ট্যাগসমূহ</h4>
              ...
              <div className="space-y-2.5">
                {Array.from({ length: course.lessons }).map((_, idx) => (
                  ...
                  >
                    ...
                    <Badge variant="outline" className="text-[9px] font-mono border-border/40 select-none">
                      locked
                    </Badge>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            <Card className="border border-primary/20 bg-card/15 backdrop-blur-xl overflow-hidden shadow-xl sticky top-24">
              <div className="absolute top-2 right-2 text-[9px] font-mono opacity-15 text-primary">enrollment.pkg</div>
              <CardContent className="pt-6 space-y-6">
                {/* Cost Header */}
                <div className="text-center pb-4 border-b border-border/20">
                  <span className="text-xs text-muted-foreground block font-mono">কোর্স ফি</span>
                  {course.isFree ? (
                    <span className="text-4xl font-extrabold font-heading text-primary block mt-1">ফ্রি</span>
                  ) : (
                ...
                <div>
                  {enrollmentStatus === "loading" ? (
                    <Button disabled className="w-full h-11 pointer-events-none font-semibold">
